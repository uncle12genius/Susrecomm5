    ...
    <header className={`fixed top-0 left-0 w-full z-50 bg-white/80 backdrop-blur-sm transition-shadow duration-300 ${isScrolled ? 'shadow-md' : ''}`}>
      <div className='container mx-auto flex justify-between items-center py-4 px-6 md:px-12 lg:px-16'>
        ...
        <div className='flex items-center gap-2'>
          <img src="./hero.png" alt="SusRecomm Logo" className='w-12 h-12 rounded-full' aria-hidden="true" />
        </div>
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`md:hidden fixed inset-0 bg-white transition-opacity z-50 ${
      ...
      >
        ...
        >
          ...
          <nav aria-label="Mobile navigation">
            <ul className='flex flex-col gap-2 px-4 text-blue-800 font-medium bg-white'> {/* Changed text color to gray-800 */}
              {NAV_ITEMS.map((item) => (
                <li key={item}>
                  <a
                    href={`#${item.replace(/\s+/g, '')}`}
